...
- **Agent-pointed-at-repo install (catalogue + one-shot installer)** — so an agent handed a link to
  ...
  *subdirectory* (it clones the URL root and requires `distribution.yaml` there), the installers do
  ...
  against a local checkout or cloning `--repo`/`$HERMES_SETUP_REPO`. `profiles.json` is
  **generated infrastructure** (regenerated by `compile`, secret-scanned, deterministic) and, like
  the installers, **embeds no repo URL / org** — the agent uses the link it was given.
