...
Personal assistant for life in Israel — gov services, consumer rights, forms, budgeting.

Generated by the hermes-setup configurator — **do not hand-edit**. Regenerate with
`python -m configurator compile il-citizen`.
...
```bash
./install.sh il-citizen          # POSIX;  --yes to skip prompts
.\install.ps1 il-citizen         # Windows; -Yes to skip prompts
```

A bare install also works (a standalone dist folder or Hermes Desktop import):

```bash
hermes profile install <path-or-git-url> --name il-citizen
```
...
hermes skills install skills-sh/skills-il/government-services/israeli-bureaucracy-decoder
```
...
- `skills-sh/skills-il/government-services/israeli-bureaucracy-decoder` — Decode Israeli gov letters/forms (Tax Authority, Bituach Leumi, municipalities) to plain language.
