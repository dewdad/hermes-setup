...
Personal assistant for an Israeli legal professional — contracts, tax, compliance, drafting, with strict disclaimers.

Generated by the hermes-setup configurator — **do not hand-edit**. Regenerate with
`python -m configurator compile il-legal`.
...
```bash
./install.sh il-legal          # POSIX;  --yes to skip prompts
.\install.ps1 il-legal         # Windows; -Yes to skip prompts
```

A bare install also works (a standalone dist folder or Hermes Desktop import):

```bash
hermes profile install <path-or-git-url> --name il-legal
```
